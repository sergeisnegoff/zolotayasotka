                        ...
                        <div class="panel-body">
                            ...
                            <ul class="permissions checkbox">
                                ...
                                @foreach(Voyager::model('Permission')->all()->groupBy('table_name') as $table => $permission)
                                    @php($tableTitle = \Illuminate\Support\Facades\DB::table('data_types')->where('name', $table)->first())
                                    <li>
                                        <input type="checkbox" id="{{$table}}" class="permission-group">
                                        <label for="{{$table}}"><strong>{{ !is_null($tableTitle) ? $tableTitle->display_name_plural : $sectionTitle[$table]  }}</strong></label>
                                        <ul>
                                            @foreach($permission as $perm)
                                                <li>
                                                    ...
                                                    <label for="permission-{{$perm->id}}">{{ \Illuminate\Support\Str::title(str_replace(['browse', 'read', 'edit', 'add', 'delete', $table], ['Просмотр', 'Чтение', 'Редактирование', 'Добавление', 'Удаление', !is_null($tableTitle) ? $tableTitle->display_name_plural : $sectionTitle[$table] ], str_replace('_', ' ', $perm->key)) )}}</label>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </li>
